...
{
    // 1. البحث عن المهمة المحددة بالـ id القادم من الفورم
    var task = tasks.FirstOrDefault(t => t.Id == id);
    
    // 2. التحقق من وجود المهمة وأن العنوان الجديد ليس فارغاً
    if (task != null && !string.IsNullOrWhiteSpace(newTitle))
    {
        // 3. تعديل عنوان هذه المهمة فقط
        task.Title = newTitle.Trim();
    }
    
    // 4. الحل السحري لمنع الكاش واختفاء العناصر: العودة إلى Index مباشرة
